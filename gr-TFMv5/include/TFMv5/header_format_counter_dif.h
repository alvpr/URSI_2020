...
#ifndef INCLUDED_TFMV5_HEADER_FORMAT_COUNTER_DIF_H
#define INCLUDED_TFMV5_HEADER_FORMAT_COUNTER_DIF_H
 ...
 */
class TFMV5_API header_format_counter_dif : public digital::header_format_default
{
public:
    typedef std::shared_ptr<header_format_counter_dif> sptr;

    header_format_counter_dif(const std::string& access_code, int threshold, int bps);
    ~header_format_counter_dif() override;


    /*!
     * Returns the number of bits set in a byte.
     * \param number The byte (uint8_t)
     */
    int bits_set(uint8_t number);
...
#endif /* INCLUDED_TFMV5_HEADER_FORMAT_COUNTER_DIF_H */
