...
#include "header_payload_demux_am_impl.h"
...
const pmt::pmt_t header_payload_demux_am_impl::msg_port_id()
{
...
header_payload_demux_am::sptr
header_payload_demux_am::make(int header_len,
...
{
    return gnuradio::make_block_sptr<header_payload_demux_am_impl>(header_len,
                                                                items_per_symbol,
                                                                guard_interval,
                                                                length_tag_key,
                                                                trigger_tag_key,
...
header_payload_demux_am_impl::header_payload_demux_am_impl(
    int header_len,
    int items_per_symbol,
    int guard_interval,
    const std::string& length_tag_key,
    ...
    const std::vector<std::string>& special_tags,
    const size_t header_padding)
    : block("header_payload_demux_am",
            io_signature::make2(1, 2, itemsize, sizeof(char)),
            io_signature::make(
                5, 5, (output_symbols ? itemsize * items_per_symbol : itemsize))),
...
void header_payload_demux_am_impl::forecast(int noutput_items,
...
bool header_payload_demux_am_impl::check_buffers_ready(int output_symbols_reqd,
...
int header_payload_demux_am_impl::general_work(int noutput_items,
                                            gr_vector_int& ninput_items,
                                            ...
                                            gr_vector_void_star& output_items)
{
    ...
    unsigned char* out_payload = (unsigned char*)output_items[p_state];
...
#define CONSUME_ITEMS(items_to_consume)                                         \
    ...
    case STATE_HEADER_RX_SUCCESS:
        ...
        for (size_t i = 0; i < d_payload_tag_keys.size(); i++) {
            add_item_tag(p_state,
                         nitems_written(p_state),
...
int header_payload_demux_am_impl::find_trigger_signal(int skip_items,
                                                   int max_rel_offset,
...
void header_payload_demux_am_impl::parse_header_data_msg(pmt::pmt_t header_data)
{
    ...
    if (pmt::is_integer(header_data)) {
        ...
        while (!pmt::is_null(dict_items)) {
            ...
            if (pmt::equal(pmt::car(this_item), d_len_tag_key)) {
                ...
                d_state = STATE_HEADER_RX_SUCCESS;
                switch(pmt::to_long(pmt::dict_ref(header_data, pmt::intern("bps"), pmt::PMT_NIL))){
                    case 1:
                        p_state = BPSK;
                        //std::cout << d_curr_payload_len<< "BPSK \n";
                    break;
                    case 2:
                        p_state = QPSK;
                        //std::cout << "QPSK \n";
...
void header_payload_demux_am_impl::copy_n_symbols(const unsigned char* in,
                                               unsigned char* out,
                                               int port,
...
void header_payload_demux_am_impl::update_special_tags(uint64_t range_start,
                                                    uint64_t range_end)
{
    if (d_track_time) {
        std::vector<tag_t> tags;
...
void header_payload_demux_am_impl::add_special_tags()
{
    if (d_track_time) {
