...
#ifndef INCLUDED_TFMV5_HEADER_PAYLOAD_DEMUX_AM_IMPL_H
#define INCLUDED_TFMV5_HEADER_PAYLOAD_DEMUX_AM_IMPL_H
...
class header_payload_demux_am_impl : public header_payload_demux_am
{
private:
    ...
    int p_state;                            //!< Current payload modulation
    ...
    //! Checks if there are enough items on the inputs and enough space on the output
    //! buffers to copy \p n_symbols symbols
    ...
    //  buffer (or -1 if none is found)
    int find_trigger_signal(int skip_items,
                            int noutput_items,
                            uint64_t base_offset,
                            const unsigned char* in_trigger);

    //! Copies n symbols from in to out, makes sure tags are propagated properly. Does
...
public:
    header_payload_demux_am_impl(const int header_len,
    ...
    ~header_payload_demux_am_impl() override;
...
#endif /* INCLUDED_TFMv5_HEADER_PAYLOAD_DEMUX_AM_IMPL_H */